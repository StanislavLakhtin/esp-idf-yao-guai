...
static lv_group_t * g;
static lv_indev_drv_t sim_encoder_drv;
...
void construct_settings_screen(void) {
  ...
  lv_indev_drv_init(&sim_encoder_drv);
  sim_encoder_drv.type = LV_INDEV_TYPE_ENCODER;
}
...
void lv_win_close_event_cb (lv_obj_t * obj, lv_event_t event) {
  switch (event) {
